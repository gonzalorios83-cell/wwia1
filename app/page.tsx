...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      // Los escenarios espaciales usan mapas y overlays grandes. Renderizarlos a
      // densidad nativa evita cuadruplicar el trabajo del canvas en pantallas HiDPI.
      const width = wrapper.clientWidth, height = wrapper.clientHeight, dpr = Math.min(isSpaceScenario() ? 1 : 2, window.devicePixelRatio || 1);
      if (canvas.width !== Math.floor(width * dpr) || canvas.height !== Math.floor(height * dpr)) { canvas.width = Math.floor(width * dpr); canvas.height = Math.floor(height * dpr); }
      const ctx = canvas.getContext("2d"); if (!ctx) return; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = isSpaceScenario() ? "medium" : "high";
